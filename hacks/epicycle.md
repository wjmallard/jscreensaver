...
## Variable-delay loop
`epicycle_draw` returns the microseconds to wait before the next call — `delay` while drawing, `holdtime` seconds at completion, and the ~1 s erase between figures. The port keeps the boxfit-style loop: `step()` returns the ms until the next step and the rAF lag-accumulator honours it.
...
- **Erase transition integrated.** The C calls xscreensaver's `erase_window` animated transition between figures; the port runs the same ~1 s wipe via `wipes.js` (the erase.c port).
