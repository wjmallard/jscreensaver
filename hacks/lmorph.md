...
- **Colour.** The C draws a single static foreground (`#4444FF`, blue). This port keeps the gallery's vivid look by **drifting the hue** each frame (`hsl(h, 90%, 62%)`), starting at hue 240 (blue) as a nod to the original. `colorSpeed` (degrees/frame, live) controls the drift; set it to 0 for an effectively static colour. The `--ncolors`-style control doesn't exist in stock lmorph, so this is an addition for parity with the other hacks.
- **Default tuning for smooth motion.** Stock is `delay 70000` / `steps 150` (≈14 fps of a fully-redrawn line — visibly chunky on a 60 Hz display). Per the brief (keep `delay` ≤ one frame and add interpolation steps rather than per-step motion), this ships `delay 16000` (≈60 fps) / `steps 400`, so a single morph still lasts ≈6.5 s but updates smoothly. Both sliders keep the xml ranges.
...
## Config
Ranges mirror `hacks/config/lmorph.xml`:
- `delay` — **Frame rate** (`--delay`, µs/step, live, inverted: drag right = faster; default retuned, see above).
- `points` — **Control points** (`--points`, 10–1000; non-live — sizes the point buffers, re-runs via `reinit()`).
- `steps` — **Interpolation steps** (`--steps`, 100–500; live — morph speed; default retuned).
- `linewidth` — **Lines** (`--linewidth`, 1–50; live).
- `figtype` — **Figures** (`--figtype` all/open/closed; non-live — sizes the figure pool).
- `colorSpeed` — **Color cycling** (added for parity; live).
