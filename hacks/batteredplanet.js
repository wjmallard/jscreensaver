// batteredplanet.js — "A battered alien planet" packaged as a mountable WebGL module.
// start(canvas) returns { stop, pause, resume, reinit, config, params }.
//
// "A battered alien planet" by mrange, 2020 — a single-pass ray-march over a
// procedurally noised alien terrain under twin suns, a banded gas giant looming
// on the horizon. Originally a Shadertoy (https://www.shadertoy.com/view/
// wsjBD3), CC0 (public domain); xscreensaver 6.x ships it as
// hacks/glx/glsl/batteredplanet.glsl run by its xshadertoy.c harness. Here it
...
// batteredplanet.glsl). See batteredplanet.md.
//
// Rendering: one full-screen triangle, one fragment-shader pass per frame, no
// textures, no multi-pass buffers. The per-pixel cost (up to 65 march steps,
// each evaluating multi-octave value noise) is exactly what makes this a WebGL
// piece and not a canvas2d one — there is no CPU-side simulation at all.

import { startShadertoy } from './shadertoy.js';

export const title = 'batteredplanet';
...
// Verbatim from hacks/glx/glsl/batteredplanet.glsl (Shadertoy wsjBD3, CC0). The
// author's #define'd parameters and constants are left exactly as tuned; the
...
export function start(canvas) {
  ...
  const config = {
    speed: 1.0,        // playback-rate multiplier on the fly-over
  ...
  return startShadertoy(canvas, { source: SHADER, config, params, name: 'batteredplanet' });
}
