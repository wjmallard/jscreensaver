...
- **`poly` cap 24** (the C's `MAXPOLY` is 16) and the xml's `high="100"` is trimmed to a saner slider range; **`count` default 2** (xml 4) and max 12, since opaque overlapping ribbons get busy fast on a bright canvas. `delay` keeps the stock value (see **Timing**).
- **Colour** is a faithful `hsv_to_rgb` port, not `hsl` — see **Palette**. `colorShift` (Color contrast) maps 1:1 as degrees-per-frame.
...
## Palette
qix is a **native screenhack** that colours each qix itself — there is no `make_*_colormap`. `init_one_qix` seeds every qix with `hsv_to_rgb(random()%360, frand(1.0), frand(0.5)+0.5)`: a random hue, a random **saturation in [0, 1)** and a random **value in [0.5, 1)**, all held for that qix's lifetime; `add_qline` then advances *only the hue* by `colorShift` each frame (`h = (h + colorShift) % 360`), holding s and v. Because the saturation is a free `frand(1.0)` roll, the live screen mixes vivid, pastel and **near-grey** ribbons (a low-S roll = a grey ribbon) — see the live non-transparent capture.

The port transcribes `utils/hsv.c`'s `hsv_to_rgb` directly (inline `hsvColor(h, s, v)`, with the X server's 16-bit→8-bit `>>8` downsample) and gives each qix a held `sat = frand(1.0)`, `val = frand(0.5)+0.5`, cycling only the hue. The earlier port used a fixed vivid `hsl(h, 100%, 55%)` — wrong colour *space* (HSV and HSL only coincide at s=v=1) **and** no per-qix S/V variety, so every ribbon came out uniformly vivid. That was the systemic vivid-rainbow bug; it is now fixed.

(jwz's default is `transparent:true`, which on a TrueColor X server draws via random RGB-bit plane masks with **no** hue cycling — a different look the port deliberately does not model, since X11 plane-mask alpha has no Canvas-2D equivalent. The port renders the non-transparent hue-cycling path, which is also what the macOS/jwxyz build and the demo video show.)

## Timing
Stock `*delay: 10000`. The live non-transparent binary reads **~54 fps at Load ~45 %** on the `-fps` overlay (two runs: 55.3 fps / 44.7 %, 53.9 fps / 46.1 %) — Load well under 100 %, so it is **delay-bound** (idle ≈ the 10 ms floor; the ~8 ms compute slice is the overhead). `OVERHEAD = round(1e6 / 54.6) − 10000 ≈ 8300 µs`, added to `config.delay` in the rAF lag-accumulator so a step runs every ≈18.3 ms (~54 steps/s) instead of racing the bare 10 ms floor (~1.8× too fast). `config.delay` itself stays at the stock 10000 and the slider maps 1:1. (Measured, not by eye; the transparent default is much heavier at ~25 fps, but the port models the non-transparent path.)
