...
- **Colors**: the C builds an X `make_color_ramp` from a random vivid hue (s=v=1) to a random dimmer hue (s=v=0.7), `ncolors` entries, indexed `1..ncolors` by the light distance. We build an `hsl()` ramp between two random hues with lightness easing 65%→50% across the ramp (vivid per the gallery convention, brighter near the light), indexed `0..ncolors-1` (the C's `gcs[color]` → our `palette[color-1]`). With `ncolors` small (default **4**) you get the original's banded, few-colour look.
...
- **`--fps`** ("Show frame rate") and the `--root` command flag are X/host concerns and omitted, as in the other ports. Tuning: `delay` left at the stock 10 ms — at the defaults it already vibrates calmly enough; the whole point of the hack is edginess, so it isn't slowed further.
...
- `delay` — Frame rate, µs/step, default 10000, `invert: true`, **live**.
...
- `lineWidth` — Line thickness, 0–20, default 0 (=1px), **non-live** (folded into the stroke; reinit harmless).
