// goldenapollian.js — "Golden apollian" packaged as a mountable WebGL module.
// start(canvas) returns { stop, pause, resume, reinit, config, params }.
//
// Golden apollian by Martin "mrange" Ranelius, 2021 — a camera flight down a
// twisting tunnel of stacked planes, each etched with a golden Apollonian-gasket
// fractal (an inversive packing of ever-smaller circles) lit by drifting suns.
// Originally a Shadertoy (https://www.shadertoy.com/view/WlcfRS), released CC0;
// xscreensaver 6.x ships it as hacks/glx/glsl/goldenapollian.glsl run by its
...
// original (kept alongside as goldenapollian.glsl). See goldenapollian.md.
//
// Rendering: one full-screen triangle, one fragment-shader pass per frame, no
// textures, no multi-pass buffers. Each pixel marches up to nine planes and
// folds an Apollonian fractal seven times per plane, so it is a genuinely heavy
// per-pixel shader and not a canvas2d one — there is no CPU-side simulation.

import { startShadertoy } from './shadertoy.js';

export const title = 'goldenapollian';
...
// Verbatim from hacks/glx/glsl/goldenapollian.glsl (Shadertoy WlcfRS, CC0). The
// author's #defines and the in-shader effect tables are left exactly as tuned;
// the harness adds only a playback-speed multiplier and a render-resolution
// scale around it. iResolution / iTime / iMouse are supplied by the harness
// preamble.
const SHADER = `
...
export function start(canvas) {
  ...
  return startShadertoy(canvas, { source: SHADER, config, params, name: 'goldenapollian' });
}
