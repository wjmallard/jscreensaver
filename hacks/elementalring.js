// elementalring.js — "Elemental Ring" packaged as a mountable WebGL module.
// start(canvas) returns { stop, pause, resume, reinit, config, params }.
//
// Elemental Ring by Otavio Good, 2016 — a single-pass ray-march of three glowing
// braids twisting around a torus ring, lit so the strands read like molten
// elemental energy, with a slow drifting orbit camera. Originally a Shadertoy
// (https://www.shadertoy.com/view/MsVXDt), CC0 (public domain); xscreensaver 6.x
// ships it as hacks/glx/glsl/elementalring.glsl run by its xshadertoy.c harness.
// Here it runs natively on WebGL2 / GLSL ES 3.00 via the shared ./shadertoy.js
// harness, and the shader body is verbatim from the original (kept alongside as
// elementalring.glsl). See elementalring.md.
//
// Rendering: one full-screen triangle, one fragment-shader pass per frame, no
// textures, no multi-pass buffers. The per-pixel cost (up to 80 march steps,
// each evaluating the braided signed-distance field) is exactly what makes this
// a WebGL piece and not a canvas2d one — there is no CPU-side simulation at all.

import { startShadertoy } from './shadertoy.js';

export const title = 'elementalring';
...
// Verbatim from hacks/glx/glsl/elementalring.glsl (Shadertoy MsVXDt, CC0). The
// author's constants and #defines are left exactly as tuned; the harness adds
...
export function start(canvas) {
  ...
  const config = {
    speed: 1.0,        // playback-rate multiplier on the orbit + braid animation
  ...
  return startShadertoy(canvas, { source: SHADER, config, params, name: 'elementalring' });
}
