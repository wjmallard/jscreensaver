// fluxcore.js — "Flux Core" packaged as a mountable WebGL module.
// start(canvas) returns { stop, pause, resume, reinit, config, params }.
//
// "Flux Core" by otaviogood, 2015 — a single-pass distance-field ray-march that
// flies through a glowing fractal reactor: nested carved cylinders spiral inward
// past rust-streaked struts and orange wobbling tori toward a blazing energy beam
// at the core. Originally a Shadertoy (https://www.shadertoy.com/view/ltlSWf),
// CC0 (public domain); xscreensaver 6.x ships it as hacks/glx/glsl/fluxcore.glsl
...
// from the original (kept alongside as fluxcore.glsl). See fluxcore.md.
//
// Rendering: one full-screen triangle, one fragment-shader pass per frame, no
// textures, no multi-pass buffers. The per-pixel cost (up to 210 march steps,
// each evaluating a deeply-nested distance field plus 30-step sun shadows and
// multi-octave 3D noise) is exactly what makes this a WebGL piece and not a
// canvas2d one — there is no CPU-side simulation at all.

import { startShadertoy } from '../shadertoy.js';

export const title = 'fluxcore';
...
// Verbatim from hacks/glx/glsl/fluxcore.glsl (Shadertoy ltlSWf, CC0). The
// author's constants and animation timeline are left exactly as tuned; the
...
// Date:   24-Aug-2015
// Desc:   Long range space-based energy transmission requires a flux core to amplify and concentrate energy.
...
export function start(canvas) {
  ...
  return startShadertoy(canvas, { source: SHADER, config, params, name: 'fluxcore' });
}
