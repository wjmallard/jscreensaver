// alienbeacon.js -- "Alien Beacon" packaged as a mountable WebGL module.
// start(canvas) returns { stop, pause, resume, reinit, config, params }.
//
// Alien Beacon by otaviogood, 2014 -- a raymarched flight through a procedural
// alien canyon: ridged spiral noise sculpts rust-colored rock and tailings
// water around a pulsing green beacon sphere, all under a drifting sun.
// Originally a Shadertoy (https://www.shadertoy.com/view/ld2SzK), CC0 / public
// domain; xscreensaver 6.x ships it as hacks/glx/glsl/alienbeacon.glsl run by
// its xshadertoy.c harness. Here it runs natively on WebGL2 / GLSL ES 3.00 via
// the shared ./shadertoy.js harness, and the shader body is verbatim from the
// original (kept alongside as alienbeacon.glsl). See alienbeacon.md.
//
// Rendering: one full-screen triangle, one fragment-shader pass per frame, no
// textures, no multi-pass buffers. The per-pixel cost (up to 200 raymarch steps
// over multi-octave spiral noise, plus a Catmull-Rom camera path) is exactly
// what makes this a WebGL piece and not a canvas2d one -- there is no CPU-side
// simulation at all.

import { startShadertoy } from '../shadertoy.js';

export const title = 'alienbeacon';
...
// Verbatim from hacks/glx/glsl/alienbeacon.glsl (Shadertoy ld2SzK, CC0). The
// author's #define'd toggles (MOTION_BLUR, MOVING_SUN) and tuning are left
// exactly as set; the harness adds only a playback-speed multiplier and a
// render-resolution scale around it. iResolution / iTime / iMouse are supplied
...
export function start(canvas) {
  ...
  return startShadertoy(canvas, { source: SHADER, config, params, name: 'alienbeacon' });
}
