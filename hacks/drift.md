...
- **No X11 GC / palette** — colour is a vivid `hsl(i/ncolors, 100%, 55%)` rainbow packed to ABGR, matching the gallery's other ports (brighter than the X colormap). `ncolors==1/2` falls back to mono (white), so the "Colors" knob still bites at the low end (mirrors the C's `dp->color = MI_NPIXELS > 2`).
- **`delay` default 20000 µs** vs. the stock 10000 — a touch calmer, per the gallery's house style. (Stock value still reachable on the slider.)
...
## Config
Units/defaults mirror `hacks/config/drift.xml`: `delay` (µs/frame, 20000 here vs xml 10000), `count` ("Duration", flame-lifetime scale, 30), `ncolors` (200). Added: `grow` and `liss` checkboxes for the original's `-grow`/`-liss` options.
