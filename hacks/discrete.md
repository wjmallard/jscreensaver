...
**Pace note / live-calibration flag.** The xml default `delay` is **20000 µs**; this port ships **50000 µs**. The structure is otherwise 1:1 (one `step()` = one `draw_discrete` = `INNER`=10 inner frames), so 20000 would reproduce the C's *nominal* rate exactly — but per the project's framerate-calibration finding the live binary's *effective* tick rate runs well below nominal (the delay is a floor plus per-tick overhead), so 50000 is a deliberate calmer default aimed at the effective pace. This is permitted by the brief's "you may pick a calmer default if documented" carve-out, **not** a fidelity claim. The main session should confirm/adjust the default against the live binary (this agent must not run it). `delay` also governs the colour-walk and map-lifetime pace, so all three move together with it. See [[hopalong]], [[thornbird]], [[squiral]].
