...
- **Timing model.** The C returns a per-call microsecond sleep derived from `speed`, `sleep`, and `lpu`. This port keeps `lpu = li/200` (li/50 on small windows) batches but paces them with the host's `delay` (Frame rate, microseconds) scaled by `speed` (the C's 0-5), and lingers `sleep` seconds after each completed screen via the variable-delay loop (like boxfit). The exact "10/8/6/4/2/0 second" total-draw targets are not reproduced; `delay`*`speed` controls batch speed instead.
