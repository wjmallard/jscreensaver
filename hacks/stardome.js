// stardome.js -- "Stars and galaxy" packaged as a mountable WebGL module.
// start(canvas) returns { stop, pause, resume, reinit, config, params }.
//
// Stars and galaxy by mrange, 2022 -- a procedural night sky cast on the inside
// of a dome: layered twinkling star fields colored by blackbody temperature, a
// tilted spiral galaxy, a lit crescent moon and a faint horizon grid, all swept
// by a slowly rotating camera. Originally a Shadertoy (https://www.shadertoy.com/
// view/stBcW1), CC0 (public domain); xscreensaver 6.x ships it as
// hacks/glx/glsl/stardome.glsl run by its xshadertoy.c harness. Here it runs
// natively on WebGL2 / GLSL ES 3.00 via the shared ./shadertoy.js harness, and
// the shader body is verbatim from the original (kept alongside as stardome.glsl).
// See stardome.md.
//
// Rendering: one full-screen triangle, one fragment-shader pass per frame, no
// textures, no multi-pass buffers. Each pixel ray-marches a few fbm noise octaves
// per star layer; that per-pixel cost is what makes this a WebGL piece and not a
// canvas2d one -- there is no CPU-side simulation at all.

import { startShadertoy } from './shadertoy.js';

export const title = 'stardome';
...
// Verbatim from hacks/glx/glsl/stardome.glsl (Shadertoy stBcW1, CC0). The
...
export function start(canvas) {
  ...
  return startShadertoy(canvas, { source: SHADER, config, params, name: 'stardome' });
}
