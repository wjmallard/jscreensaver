...
- **Default `delay` 30000 µs** — unchanged from stock; the look is already calm. `count` defaults to 3 (xml min is 0, but the C clamps `count < 1 → 1`, so the param min is 1).
