...
- **`delay` default.** xml stock is `20000` µs; the port defaults to `40000` to match the C's *effective* ~half-nominal step rate (xscreensaver's delay is a floor plus per-frame overhead). A pace knob, not a fidelity item.
