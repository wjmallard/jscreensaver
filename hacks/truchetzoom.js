// truchetzoom.js — "Another truchet experiment" packaged as a mountable WebGL module.
// start(canvas) returns { stop, pause, resume, reinit, config, params }.
//
// "Another truchet experiment" by mrange, 2024 — an infinitely self-similar
// truchet weave that zooms forever down a golden-ratio (phi) boxed spiral while
// a Smith-chart conformal warp swirls the whole tiling, so the pattern endlessly
// folds into itself. Originally a Shadertoy (https://www.shadertoy.com/view/
// 4cBcDy), released CC0; xscreensaver 6.x ships it as
// hacks/glx/glsl/truchetzoom.glsl run by its xshadertoy.c harness. Here it runs
// natively on WebGL2 / GLSL ES 3.00 via the shared ./shadertoy.js harness, and
// the shader body is verbatim from the original (kept alongside as
// truchetzoom.glsl). See truchetzoom.md.
//
// Rendering: one full-screen triangle, one fragment-shader pass per frame, no
// textures, no multi-pass buffers. All the structure is analytic signed-distance
// math evaluated per pixel — there is no CPU-side simulation at all.

import { startShadertoy } from './shadertoy.js';

export const title = 'truchetzoom';
...
// Verbatim from hacks/glx/glsl/truchetzoom.glsl (Shadertoy 4cBcDy, CC0). The
// author's #defines (including the DISTORT warp) are left exactly as tuned; the
// harness adds only a playback-speed multiplier and a render-resolution scale
// around it. iResolution / iTime / iMouse are supplied by the harness preamble.
const SHADER = `
...
export function start(canvas) {
  ...
  return startShadertoy(canvas, { source: SHADER, config, params, name: 'truchetzoom' });
}
