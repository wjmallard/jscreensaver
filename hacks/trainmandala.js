// trainmandala.js — "Train Mandala" packaged as a mountable WebGL module.
// start(canvas) returns { stop, pause, resume, reinit, config, params }.
//
// "Quick hack on the train" by mrange, 2023 — a kaleidoscopic mandala built
// from nested rings and radial spokes that zoom forever inward on a 10-fold
// symmetric, log-spiral lattice, glowing in shifting rainbow hues. Originally a
// Shadertoy (https://www.shadertoy.com/view/mtjyz1), released CC0; xscreensaver
// 6.x ships it as hacks/glx/glsl/trainmandala.glsl run by its xshadertoy.c
// harness. Here it runs natively on WebGL2 / GLSL ES 3.00 via the shared
// ./shadertoy.js harness, and the shader body is verbatim from the original
// (kept alongside as trainmandala.glsl). See trainmandala.md.
//
// Rendering: one full-screen triangle, one fragment-shader pass per frame, no
// textures, no multi-pass buffers. All the structure — the polar fold, the
// exponential ring spacing, the additive glow — is recomputed per pixel every
// frame; there is no CPU-side simulation at all.

import { startShadertoy } from './shadertoy.js';

export const title = 'trainmandala';
...
// Verbatim from hacks/glx/glsl/trainmandala.glsl (Shadertoy mtjyz1, CC0). The
// author's tuning is left exactly as written; the harness adds only a
...
export function start(canvas) {
  ...
  const config = {
    speed: 1.0,        // playback-rate multiplier on the zoom
  ...
  return startShadertoy(canvas, { source: SHADER, config, params, name: 'trainmandala' });
}
