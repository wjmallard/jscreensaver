...
The board is repainted each active step: filled cells in their piece colour (a muted **smooth colormap** — see Palette), blanks black, a thin dark separator on every edge between differing cells, and a pale board outline. This is essentially the C's **non-bitmap** path (`draw_without_bitmaps`: flat rectangles + boundary lines). The C's *default* path renders rounded, beveled, 3-D-looking "puzzle-piece" bitmaps (`create_bitmaps`, the `HALFBIT`/`THREEQUARTERSBIT`/bellybutton machinery, ~250 lines of per-pixel mask math); because those 1-bit dither masks blend each piece's colour ~50% with black, the live tiles read as dark-grey 3-D solids with brighter coloured rims. That look is **approximated** by flat colour cells, not reproduced; replicating it pixel-exact in canvas was deemed out of scope. (Consequence: the port's flat cells are brighter/more saturated than the live's dithered interiors, but the palette *character* — which hues, muted vs. vivid — now matches.) The piece colours, layout, board sizing, portrait rotation, and margins all match the C.
...
- **Mono / `use3D` / `plain` paths dropped.** The C picks a b&w fallback when fewer than 12 colors and four 3-D shading styles; we always colour pieces from the smooth colormap (see Palette). `--identical` is exposed as a checkbox (the C's fullrandom mode randomizes it; we honor the toggle).
