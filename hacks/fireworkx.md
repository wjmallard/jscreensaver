...
- **`delay` default 50000 µs** (stock 10000) — calmer **pace knob**, NOT a fidelity item. Note `delay` drives *simulation* speed, not just refresh: each displayed frame runs `FTWEAK = 12` physics sub-steps, so sim-substeps/sec = 12 × (1e6/delay). Under the project's framerate model (effective period ≈ delay + ~30 ms overhead → stock fireworkx ≈ 1e6/40000 ≈ 25 fps; and fireworkx is heavy, so likely lower on a big screen), 50000 µs (≈20 fps) is in the right neighbourhood. **Calibrate the exact number against the live binary.** `MAX_CATCHUP_STEPS = 3` keeps a backgrounded tab from firing a burst of blurs on refocus.
...
- `delay` — **Frame rate** (µs/step, default 50000, `live`, inverted: drag right = faster). See the pace-knob note above.
