// neontriangulator.js — "Neon Triangulator" packaged as a mountable WebGL module.
// start(canvas) returns { stop, pause, resume, reinit, config, params }.
//
// Neon Triangulator by mrange, 2025 — a synthwave ray-march down an endless
// neon-lit corridor of triangulated, reflecting prisms, with glowing edges and
// a setting sun on the horizon. Originally a Shadertoy (https://www.shadertoy.
// com/view/tXGGRD), released CC0 (public domain); xscreensaver 6.x ships it as
// hacks/glx/glsl/neontriangulator.glsl run by its xshadertoy.c harness. Here it
...
// neontriangulator.glsl). See neontriangulator.md.
//
// Rendering: one full-screen triangle, one fragment-shader pass per frame, no
// textures, no multi-pass buffers. The per-pixel cost (up to 4 reflective
// bounces, each a 77-step ray-march of a polar-folded distance field) is what
// makes this a WebGL piece and not a canvas2d one — there is no CPU-side
// simulation at all, and it is the heaviest shader in the pool so far.

import { startShadertoy } from './shadertoy.js';

export const title = 'neontriangulator';
...
// Verbatim from hacks/glx/glsl/neontriangulator.glsl (Shadertoy tXGGRD, CC0).
// The author's #defines and const parameters are left exactly as tuned; the
...
export function start(canvas) {
  ...
  return startShadertoy(canvas, { source: SHADER, config, params, name: 'neontriangulator' });
}
