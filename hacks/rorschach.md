...
- After the linger, run the erase wipe, then `startBlot()` — pick a new random hue, recentre, reset `remaining`. (The C starts its eraser and calls `rorschach_draw_start` on that same tick.)
...
- **Erase transition integrated.** The C erases the finished blot with xscreensaver's `erase_window` wipe (a random one; the 1997 change made the helix wipe available to it) before the next walk; the port runs the same ~1 s transition via `wipes.js`, seeding the next blot as it lands, like the C's `rorschach_draw_start`.
