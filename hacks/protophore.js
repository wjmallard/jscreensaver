// protophore.js — "Protophore" packaged as a mountable WebGL module.
// start(canvas) returns { stop, pause, resume, reinit, config, params }.
//
// Protophore by otaviogood, 2015 — a four-level recursive sphere fractal whose
// cubic "teeth" interlock and rotate, ray-marched with one bounce of reflection
// and lit by a procedural overhead softbox (the sports-car-photography look).
//
// Rendering: one full-screen triangle, one fragment-shader pass per frame, no
// textures, no multi-pass buffers (the commented-out cube-map
// code aside, the live environment is fully procedural). Each pixel ray-marches the
// scene, so this is a true WebGL piece with no CPU-side simulation at all.

import { startShadertoy } from './shadertoy.js';

export const title = 'protophore';
...
// Verbatim from hacks/glx/glsl/protophore.glsl (Shadertoy XljGDz, CC0).
// As in gimbalharmonics, the texture lookups are in commented-out GetEnvColor
// helpers; the live code uses the procedural GetEnvColor2, so this needs no
// channels. SPLIT_ANIM is left undefined, exactly as shipped.
const SHADER = `
// Title:  Protophore
// Author: otaviogood
...
export function start(canvas) {
  ...
  return startShadertoy(canvas, { source: SHADER, config, params, name: 'protophore' });
}
