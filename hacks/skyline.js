// skyline.js — "Skyline" packaged as a mountable WebGL module.
// start(canvas) returns { stop, pause, resume, reinit, config, params }.
//
// Skyline by otaviogood, 2015 — a procedurally generated night city flown on
// rails: ray-marched skyscrapers with lit windows, cars with glowing trails, and
// a hazy reflective skyline, all from analytic distance fields.
//
// Rendering (harness v2, multipass): a single ray-march pass that samples its OWN
// previous frame on iChannel0. The original Shadertoy bound iChannel0 to a cube
// map for the reflections on cars and glass; xscreensaver can't load textures, so
// xshadertoy.c feeds the pass its last frame instead, and we reproduce that with
// a self-feedback (ping-pong) buffer. The shader body is verbatim (the only
// channel edit, "texture(iChannel0, ref.xy)" in place of the cube-map "ref", is
// xscreensaver's own GLES3 fix, already present in skyline.glsl).

import { startShadertoy } from './shadertoy.js';

export const title = 'skyline';
...
// Verbatim from hacks/glx/glsl/skyline.glsl (Shadertoy XtsSWs, CC0).
const SHADER = `
// Title:  Skyline
// Author: otaviogood
// URL:    https://www.shadertoy.com/view/XtsSWs
// Date:   23-Sep-2015
// Desc:   I have always wanted to make a procedural city. The architecture was limited by what I could do in real-time, but I tried to give it enough complexity that it would look realistic.
...
// jwz: GLSL 4.10, GLES 3.0: "No matching function for call to texture(sampler2D, vec3)"
//     This must just be an error; dunno why it works on the shadertoy web site.
...
export function start(canvas) {
  ...
  // One pass that reflects its own previous frame (iChannel0 = 'self'); the
  // harness ping-pongs the buffer and blits the result to the screen.
  return startShadertoy(canvas, {
    passes: [
      { name: 'image', source: SHADER, channels: { 0: 'self' } },
    ],
    config,
    params,
    name: 'skyline',
