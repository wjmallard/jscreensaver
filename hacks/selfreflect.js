// selfreflect.js -- "Let's self reflect" packaged as a mountable WebGL module.
// start(canvas) returns { stop, pause, resume, reinit, config, params }.
//
// "Let's self reflect" by mrange, 2024 -- a single Platonic solid built from a
// knighty-style polyhedral fold, ray-marched and traced through a glassy inner
// sphere so its mirrored facets reflect into themselves, bounce after bounce.
// Originally a Shadertoy (https://www.shadertoy.com/view/XfyXRV), released CC0;
// xscreensaver 6.x ships it as hacks/glx/glsl/selfreflect.glsl run by its
...
// original (kept alongside as selfreflect.glsl). See selfreflect.md.
//
// Rendering: one full-screen triangle, one fragment-shader pass per frame, no
// textures, no multi-pass buffers. The per-pixel cost (an outer 90-step march
// plus up to 6 refracted/reflected inner bounces of 50 steps each) is exactly
// what makes this a WebGL piece and not a canvas2d one -- there is no CPU-side
// simulation at all, and it is heavy enough that the resolution knob matters.

import { startShadertoy } from './shadertoy.js';

export const title = 'selfreflect';
...
// Verbatim from hacks/glx/glsl/selfreflect.glsl (Shadertoy XfyXRV, CC0). The
// author's const/#define parameters are left exactly as tuned; the harness adds
...
const SHADER = `// Title:  Let's self reflect
// Author: mrange
// URL:    https://www.shadertoy.com/view/XfyXRV
// Date:   11-May-2024
// Desc:   CC0: Let's self reflect
...
export function start(canvas) {
  ...
  const config = {
    speed: 1.0,        // playback-rate multiplier on the rotation
  ...
  return startShadertoy(canvas, { source: SHADER, config, params, name: 'selfreflect' });
}
