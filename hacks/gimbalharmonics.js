// gimbalharmonics.js — "Gimbal Harmonics" packaged as a mountable WebGL module.
// start(canvas) returns { stop, pause, resume, reinit, config, params }.
//
// Gimbal Harmonics by otaviogood, 2015 — fifteen discs nested inside one
// another, each spun on its own axis so they sweep through harmonic patterns
// around a hot pink core, ray-marched and lit with a procedural studio softbox.
//
// Rendering: one full-screen triangle, one fragment-shader pass per frame, no
// textures, no multi-pass buffers (despite the commented-out
// cube-map code, the live environment is fully procedural). Each pixel ray-marches the
// scene, so this is a true WebGL piece with no CPU-side simulation at all.

import { startShadertoy } from './shadertoy.js';

export const title = 'gimbalharmonics';
...
// Verbatim from hacks/glx/glsl/gimbalharmonics.glsl (Shadertoy llS3zd, CC0).
// The author's only texture lookups live in commented-out GetEnvColor helpers;
// the active code uses the procedural GetEnvColor2, so this needs no channels.
const SHADER = `
...
export function start(canvas) {
  ...
  return startShadertoy(canvas, { source: SHADER, config, params, name: 'gimbalharmonics' });
}
