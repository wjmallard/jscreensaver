// synthwavecity.js — "Synthwave City" packaged as a mountable WebGL module.
// start(canvas) returns { stop, pause, resume, reinit, config, params }.
//
// Synthwave City by 3w36zj6, 2022 — a pure signed-distance scene that paints a
// retro-80s skyline: a banded neon sun, a scrolling perspective grid, and a
// parade of magenta skyscrapers drifting past against a pink-and-violet haze.
// Originally a Shadertoy (https://www.shadertoy.com/view/7lKyDD), itself a
// reworking of Jan Mroz's "Synthwave" (CC BY 3.0); xscreensaver 6.x ships it as
// hacks/glx/glsl/synthwavecity.glsl run by its xshadertoy.c harness. Here it
...
// synthwavecity.glsl). See synthwavecity.md.
//
// Rendering: one full-screen triangle, one fragment-shader pass per frame, no
// textures, no multi-pass buffers. Each pixel evaluates a handful of analytic
// SDFs (sun, grid, sixteen skyscrapers), so this is a true WebGL piece with no
// CPU-side simulation at all.

import { startShadertoy } from './shadertoy.js';

export const title = 'synthwavecity';
...
// Verbatim from hacks/glx/glsl/synthwavecity.glsl (Shadertoy 7lKyDD; derived
// from Shadertoy 3t3GDB, CC BY 3.0). The author's helpers and constants are
// left exactly as written; the harness adds only a playback-speed multiplier
// and a render-resolution scale around it. iResolution / iTime / iMouse are
// supplied by the harness preamble.
const SHADER = `
...
// Original Author: Jan Mroz (jaszunio15)
...
export function start(canvas) {
  ...
  const config = {
    speed: 1.0,        // playback-rate multiplier on the scroll/animation
  ...
  return startShadertoy(canvas, { source: SHADER, config, params, name: 'synthwavecity' });
}
