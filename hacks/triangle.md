...
- **Inter-scene dwell restored**: xscreensaver's STANDALONE build clears and restarts the instant the finest pass completes (no pause -- you never linger on the finished range). The C's xlockmore path *did* pause (`MI_PAUSE = 2000000` us = 2 s). We restore that as a **Dwell** slider (in steps, default 200 ~= 2 s at the default delay), then wipe and regenerate.
...
- **Tuning / dropped extras**: `delay` stays us (xml default 10000, kept). The `--fps` flag, the multi-screen `tp->fast` field (set but unused in the C), and the X expose/`refresh` repaint path are dropped (host owns the meter; canvas needs no manual repaint).
