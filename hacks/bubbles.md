...
- **[C, deliberate, small] Calmer default delay.** The xml stock `delay` is `10000` µs (nominal ~100 fps). Per the project's frame-rate calibration, xscreensaver's *effective* rate is roughly half nominal (per-frame overhead), so the default here is `20000` µs to reproduce the C's effective 5-bubbles/frame cadence rather than running ~2× too fast. Pace knob only; the slider still exposes the full `0–100000` range.
