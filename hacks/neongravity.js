// neongravity.js — "Neon Gravity" packaged as a mountable WebGL module.
// start(canvas) returns { stop, pause, resume, reinit, config, params }.
//
// Neon Gravity ("Abstract gravitational well II") by Marten Range (mrange),
// 2024 — luminous neon strands spiral and plunge into a dark gravitational core,
// rendered with tone-mapping and then antialiased by a separate FXAA pass.
//
// Rendering (harness v2, multipass): two passes. SCENE ray-marches the well into
// an offscreen buffer; IMAGE runs FXAA over it (sampling the scene on iChannel0)
// and draws to the screen. NOTE: this is the original Shadertoy's wiring
// (BufferA -> Image). xscreensaver's xshadertoy.c packs the two files in name
// order and hard-wires iChannel0 to the first pass, which makes its FXAA read its
// own empty buffer and shows the scene un-antialiased; binding the channel
// explicitly here restores the author's intent. Both shader bodies are verbatim
// (the scene is neongravity-1.glsl, the FXAA is neongravity-0.glsl).

import { startShadertoy } from './shadertoy.js';

export const title = 'neongravity';
...
// Verbatim from hacks/glx/glsl/neongravity-1.glsl (Shadertoy 43G3Wc, CC0).
const SCENE = `
...
// Verbatim from hacks/glx/glsl/neongravity-0.glsl (FXAA by XorDev; GM_FXAA).
const IMAGE = `
// Title:  Abstract gravitational well II
// Author: mrange
// URL:    https://www.shadertoy.com/view/43G3Wc
