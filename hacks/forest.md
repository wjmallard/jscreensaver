...
- **Default `delay` 250000 us** vs the stock 500000. A forest is 31 steps (24 trees + a ~7-frame linger/reseed), so at stock pace one cycle takes ~15 s; halving the delay (~8 s/forest) keeps it calm but alive. The xml's `delay` slider (Frame rate, inverted) and `ncolors` (Colors) are both kept; `--fps` is dropped (the host owns the meter).
...
- **The clear/reseed cycle fires.** Same harness saw **7 full-screen clears over 200 frames** (~one per 31-frame cycle) -- the linger->`init_trees`->reseed path runs and never stalls on a full screen. Frame mix was 157 drawing / 43 blank (linger+reseed), matching the expected 24-draw + ~7-blank cadence.
...
## Config
Mirrors `hacks/config/forest.xml`: `delay` (Frame rate, live, inverted, 0..3000000 us, default lowered to 250000) and `ncolors` (Colors, reinit, 1..20). Non-live changes / "Reset to defaults" / `r` re-run `init()` (fresh forest).
