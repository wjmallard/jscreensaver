// prococean.js — "Very fast procedural ocean" packaged as a mountable WebGL module.
// start(canvas) returns { stop, pause, resume, reinit, config, params }.
//
// Very fast procedural ocean by afl_ext, 2017 — a ray-marched sea rendered from
// summed sine-wave octaves (no noise textures), lit by a cheap analytic sky and
// a roaming sun. Originally a Shadertoy (https://www.shadertoy.com/view/MdXyzX),
// MIT-licensed; xscreensaver 6.x ships it as hacks/glx/glsl/prococean.glsl run by
...
// original (kept alongside as prococean.glsl). See prococean.md.
//
// Rendering: one full-screen triangle, one fragment-shader pass per frame, no
// textures, no multi-pass buffers. The per-pixel cost (64 raymarch steps, each
// summing 12 wave octaves, plus a 36-octave normal) is exactly what makes this a
// WebGL piece and not a canvas2d one — there is no CPU-side simulation at all.

import { startShadertoy } from './shadertoy.js';

export const title = 'prococean';
...
// Verbatim from hacks/glx/glsl/prococean.glsl (Shadertoy MdXyzX, MIT). The
...
export function start(canvas) {
  ...
  const config = {
    speed: 1.0,        // playback-rate multiplier on the wave animation
  ...
  return startShadertoy(canvas, { source: SHADER, config, params, name: 'prococean' });
}
